...
def main():
    ...
    df = df[(df.type == "conventional") & (df.region == "Albany")][["Date", "Total Volume"]]
    # df.sort_values(by="Date", inplace=True)
    # df = df.reset_index(drop=True)

    # scaler = StandardScaler()
    # df["Total Volume"] = scaler.fit_transform(np.array(df["Total Volume"]).reshape(-1, 1))
    ...
    ts = TimeSeries(df, y='Total Volume', date_col="Date",
                    datetime_transform= ["yearly"],
                    lags=12, future_period=4, future_freq="weeks",
                    test_size=24)
